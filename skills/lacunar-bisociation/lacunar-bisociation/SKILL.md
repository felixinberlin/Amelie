...
Look for it in `/mnt/project/` (possibly with a `claude_` prefix) or in uploads. If it is missing, start from `assets/bisoziation-log.md` and say so in one sentence. If Amélie state files exist (`amelie-pruefprotokoll.md`, `amelie-suchplaybook.md`, `amelie-quellen.md`), read them too. The *Besetzungsatlas* tells you which fields are already crowded (`dicht`), and the source list gives you anchors.
...
- Write the full file to `/mnt/user-data/outputs/` and deliver it with `present_files`.
