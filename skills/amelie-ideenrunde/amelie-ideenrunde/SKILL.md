...
Die Dateien liegen in `/mnt/project/` (Präfix `claude_` möglich). Fehlt eine, die Fassung aus `assets/` nehmen und das dem Nutzer in einem Satz sagen.
...
Alle geänderten Zustandsdateien **vollständig** neu ausgeben (Nutzer will ganze Dateien, lädt sie ins Projekt hoch). Konkret:
...
Dateien nach `/mnt/user-data/outputs/` schreiben, mit `present_files` ausliefern.

### 7 · Methode verbessern (nur wenn verdient)

Hat die Runde etwas über die **Methode** gelernt, das in drei Runden noch stimmt (ein neues Suchmuster, eine Stoppregel, ein Quellentyp), dann `references/suchmethode.md` bzw. diese SKILL.md in einer Kopie unter `/tmp/amelie-ideenrunde/` anpassen und als aktualisiertes `.skill` paketieren (siehe `references/skill-update.md`). Rundenspezifisches gehört in die Zustandsdateien, nicht in die Skill.
