...
**Vollzug 24.09.2026 (mit dem Friedhof):** Die Dose ist aus `DOSEN_DATA` in `DISCARDED_DATA` gewandert, die Seite liegt als Grabbeigabe in `08-friedhof/grabbeigaben/crack-flora-watcher.md`, und die beiden fertigen Mails (Mail 8 an #Krautschau/Senckenberg, Mail 9 an Flora Incognita, eine davon „sofort versendbar") sind aus `src/data/deliveries.ts` gelöscht. Bis dahin hätte man einer toten Idee zwei Briefe hinterherschicken können. Damit ist auch der offene Punkt „Entscheidung Félix" vom 21.09. erledigt.
...
**Vollzug:** Aus `DOSEN_DATA` nach `DISCARDED_DATA` verschoben. Die Seite wurde zuerst gelöscht (alte Regel 7) und mit dem Friedhof als Grabbeigabe wiederhergestellt: `08-friedhof/grabbeigaben/paragraphen-dolmetscher.md`.

---

## Das Muster hinter den ersten vieren (Stand Runde 1)

*Die Auswertung über alle Gräber steht jetzt in `08-friedhof/README.md`.*
