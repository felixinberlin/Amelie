...
- **Nächstes Mal:** (1) Vor jeder Dev-Tooling-Prüfung die Suchschritte „<Funktion> security vendor / SCA" und „<Funktion> arXiv ICSE" einschließen. (2) Wenn die Runde-4-Worktrees (`worktree-bisociation-altbau-uncertainty`, `worktree-bisociation-r3-altbau-input`) gemergt werden: Protokoll, Bisoziation-Log und Dosenverzeichnis erneut abgleichen — Researcher #3 nennt eine neue Dose `05-dosen/waermesignatur.md`, die auf `main` noch nicht existiert. (3) ~~Diffgeist-, Ghost-Replay- und Pin-Tumbler-Dose um die neuen Nachbarn ergänzen~~ — erledigt am 19.09.2026 (deutsch und `en/`), für Altbau Thermal wurden meine Edits beim Merge mit PR #8 zugunsten der dort überarbeiteten Dose verworfen. **Bewusst nicht übernommen** (Design-Vorschläge oder ungeprüfte Behauptungen, Entscheidung bei Félix): Messbudget/Value-of-Information-Idee, „Streit-Ebene“-Ausgabe, Wärmesignatur aus Monatswerten als Antwort auf das Grundriss-Risiko, Sommer-Satz, Prebound-Zahlen 17 %/40 %, Thesim/HRC-Hitzetool, Verbraucherzentrale-Reichweite, Datenlizenz von EnergyMap.
