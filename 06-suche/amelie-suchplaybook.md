...
| 3 (Researcher #1, ideenrunde) | 18.09.2026 | 4 | 1 | 1 | 1 | 1 |
...
Runde 3 (Researcher #1) lag bei 50 % „frei + verengt" (2 von 4) — eines der vier Urteile (Tafel-Warenannahme) musste nach parallelem Abgleich mit Researcher #2 von `verengt` auf `besetzt` korrigiert werden (siehe Retro). Ohne diesen Abgleich wäre die Quote fälschlich bei 75 % ausgewiesen worden.
...
- **Fehler (parallel entdeckt):** Meine eigene Empfänger-Suche zu „Tafel-Warenannahme per Foto" fand nur Routenplanung + Lieferscheine und übersah, dass dieselbe Fahrer-App auch Foto-Qualitätserfassung der Ware macht — Researcher #2 und ein dritter Konflikt-Resolver haben das unabhängig richtiggestellt (`besetzt` statt `verengt`). Lehre: bei „Empfänger hat eigenes Digitalisierungsprojekt"-Treffern den vollen Funktionsumfang der App/Plattform prüfen, nicht nur die zuerst gefundene Pressemitteilung — sonst wird ein Teil-Feature übersehen, das die Idee doch abdeckt. Zwei Ideenrunde-Researcher parallel laufen zu lassen hat den Fehler diesmal sichtbar gemacht; das spricht dafür, bei Rechecks künftig kurz in `amelie-pruefprotokoll.md` nachzusehen, ob eine parallele Session dieselbe Idee schon (anders) verifiziert hat, bevor man pusht.
