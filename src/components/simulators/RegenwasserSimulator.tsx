      ...
      <div className="lg:col-span-6 bg-white rounded-2xl border border-stone-200 p-6 shadow-xs space-y-5">
        <div className="flex items-center justify-between pb-3 border-b border-stone-100">
          <h3 className="font-serif-title font-bold text-stone-900 text-lg flex items-center gap-2">
            ...
            <span>{lang === 'de' ? 'Zisterne: Erntemenge & Trockenheits-Autonomie' : lang === 'es' ? 'Medidor de captación de lluvia y resiliencia a la sequía' : 'Rain Harvesting & Drought Resilience Gauge'}</span>
          </h3>
        ...
        <div>
          <div className="flex justify-between text-xs font-mono-code mb-1">
            <span className="text-stone-600">{lang === 'de' ? 'Wirksame Dachgrundfläche:' : lang === 'es' ? 'Superficie de tejado:' : 'Catchment Roof Area:'}</span>
          ...
          <div className="flex justify-between text-3xs text-stone-400 font-mono-code mt-0.5">
            <span>20 m² ({lang === 'de' ? 'Schuppen' : lang === 'es' ? 'Caseta' : 'Shed'})</span>
            <span>80 m² ({lang === 'de' ? 'Reihenhaus' : lang === 'es' ? 'Adosado' : 'Row House'})</span>
            <span>200 m² ({lang === 'de' ? 'Einfamilienhaus' : lang === 'es' ? 'Unifamiliar' : 'Detached'})</span>
          </div>
        </div>
        ...
        <div>
          <div className="flex justify-between text-xs font-mono-code mb-1">
            <span className="text-stone-600">{lang === 'de' ? 'Mittlerer Jahresniederschlag:' : lang === 'es' ? 'Índice de lluvia anual:' : 'Annual Rainfall Index:'}</span>
            <span className="font-bold text-blue-900">{rainIndexMm} mm / {lang === 'de' ? 'Jahr' : lang === 'es' ? 'año' : 'yr'}</span>
          </div>
          <input
            type="range"
            min="450"
          ...
          />
          <div className="flex justify-between text-3xs text-stone-400 font-mono-code mt-0.5">
            <span>480 mm ({lang === 'de' ? 'Berlin / Brandenburg' : lang === 'es' ? 'Este seco' : 'Dry East'})</span>
            <span>750 mm ({lang === 'de' ? 'Köln / Frankfurt' : lang === 'es' ? 'Centro' : 'Central'})</span>
            <span>1100 mm ({lang === 'de' ? 'Voralpenland' : lang === 'es' ? 'Borde alpino' : 'Alpine Rim'})</span>
          </div>
        </div>
        ...
        <div>
          <label className="block text-xs font-bold uppercase tracking-wider font-mono-code text-stone-600 mb-1.5">
            {lang === 'de' ? 'Dach-Eindeckung (Abflussbeiwert):' : lang === 'es' ? 'Material de cubierta:' : 'Roof Surface Material:'}
          </label>
          <div className="grid grid-cols-3 gap-2 text-xs">
            {[
              ...
              >
                ...
                <div className="text-3xs text-blue-700 font-mono-code font-bold mt-0.5">{item.coeff} {lang === 'de' ? 'Ertrag' : lang === 'es' ? 'escorrentía' : 'drain'}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        <div>
          <div className="flex justify-between text-xs font-mono-code mb-1">
            <span className="text-stone-600">{lang === 'de' ? 'Zisternen- / Speichervolumen:' : lang === 'es' ? 'Volumen del depósito:' : 'Cistern Storage Volume:'}</span>
          ...
          />
          <div className="flex justify-between text-3xs text-stone-400 font-mono-code mt-0.5">
            <span>300 L ({lang === 'de' ? 'Regentonne' : lang === 'es' ? 'Barril' : 'Barrel'})</span>
            <span>1.500 L ({lang === 'de' ? 'Kompakt-Tank' : lang === 'es' ? 'Modular' : 'Modular'})</span>
            <span>5.000 L ({lang === 'de' ? 'Erdzisterne' : lang === 'es' ? 'Enterrado' : 'Underground'})</span>
          </div>
        </div>
        ...
        <div className="p-3.5 rounded-xl bg-stone-50 border border-stone-200 space-y-3">
          <label className="flex items-center gap-2 cursor-pointer text-xs font-medium text-stone-800">
            ...
            />
            <span>{lang === 'de' ? 'WC-Spülung angeschlossen (spart ~60 L Trinkwasser täglich)' : lang === 'es' ? 'Conectado a la cisterna del inodoro (~60 L de ahorro diario)' : 'Connected to toilet flushing (~60 L daily savings)'}</span>
          </label>

          <div>
            <div className="flex justify-between text-xs font-mono-code mb-1">
              <span className="text-stone-600">{lang === 'de' ? 'Gartenfläche zur Bewässerung:' : lang === 'es' ? 'Superficie de jardín regada:' : 'Irrigated Garden Area:'}</span>
      ...
      <div className="lg:col-span-6 bg-stone-900 text-white rounded-2xl p-6 shadow-md space-y-6">
        <div className="flex items-center justify-between pb-3 border-b border-stone-800">
          <div className="text-xs font-mono-code uppercase tracking-wider text-blue-400 font-bold flex items-center gap-1.5">
            ...
            <span>{lang === 'de' ? 'Wasserbilanz & Trockenheitspuffer' : lang === 'es' ? 'Balance hídrico y autonomía en sequía' : 'Water Balance & Drought Autonomy'}</span>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-center">
          <div className="bg-stone-800/80 p-3.5 rounded-xl border border-stone-700/80">
            <div className="text-2xs text-stone-400 font-mono-code uppercase">{lang === 'de' ? 'Jahres-Ernte' : lang === 'es' ? 'Captación anual' : 'Annual Harvest'}</div>
          ...
          <div className="bg-stone-800/80 p-3.5 rounded-xl border border-stone-700/80">
            <div className="text-2xs text-stone-400 font-mono-code uppercase">{lang === 'de' ? 'Trocken-Puffer' : lang === 'es' ? 'Días de sequía' : 'Drought Days'}</div>
            <div className="text-2xl font-bold font-mono-code text-emerald-400 mt-1">{droughtAutonomyDays}</div>
            <div className="text-3xs text-stone-400">{lang === 'de' ? 'Tage ohne Regen' : lang === 'es' ? 'días de autonomía' : 'days autonomy'}</div>
          </div>

          <div className="bg-stone-800/80 p-3.5 rounded-xl border border-stone-700/80 col-span-2 sm:col-span-1">
            <div className="text-2xs text-stone-400 font-mono-code uppercase">{lang === 'de' ? 'Kostenersparnis' : lang === 'es' ? 'Dinero ahorrado' : 'Money Saved'}</div>
            <div className="text-2xl font-bold font-mono-code text-amber-400 mt-1">{annualWaterSavingsEur} €</div>
            <div className="text-3xs text-stone-400">{lang === 'de' ? 'Trink- & Abwasser' : lang === 'es' ? 'tarifa de agua / año' : 'water tariff / yr'}</div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-stone-800/60 border border-stone-700 space-y-3">
          <div className="flex justify-between items-center text-xs">
            <span className="text-stone-300 font-medium">
              {lang === 'de' ? 'Sommerlicher Trockenheits-Schutzfaktor:' : lang === 'es' ? 'Protección ante olas de calor:' : 'Summer Heatwave Protection Level:'}
            </span>
            <span className={`font-mono-code font-bold px-2 py-0.5 rounded text-3xs ${
              ...
              {droughtAutonomyDays >= 21
                ? (lang === 'de' ? '🟢 Voll krisenfest (> 3 Wochen)' : lang === 'es' ? '🟢 Totalmente resiliente (> 3 semanas)' : '🟢 Fully resilient (> 3 wks)')
                : droughtAutonomyDays >= 10
                ? (lang === 'de' ? '🟡 Ausreichend (1-2 Wochen)' : lang === 'es' ? '🟡 Moderada (1-2 semanas)' : '🟡 Moderate (1-2 wks)')
                : (lang === 'de' ? '🔴 Kritisch klein (< 10 Tage)' : lang === 'es' ? '🔴 Depósito demasiado pequeño' : '🔴 Tank too small')}
            </span>
          </div>
        ...
        <div className="p-3.5 rounded-xl bg-blue-950/40 border border-blue-800/40 text-xs text-blue-200 leading-relaxed font-serif-title">
          <p>
            {lang === 'de'
              ...
              : lang === 'es' ? '💧 Ventaja municipal: captar la lluvia in situ evita las tasas por aguas pluviales en la mayoría de ciudades europeas (1,50 – 2,20 € por m² de tejado sellado al año) y protege el acuífero local.' : '💧 Municipal Advantage: Harvesting rainfall on-site avoids stormwater runoff fees in most European cities (saving 1.50 - 2.20 €/m² of sealed roof area every year), while safeguarding local groundwater tables.'}
          </p>
        </div>
      </div>
    </div>
