        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-stone-300 mt-1 max-w-2xl leading-relaxed">
              {isDe
                ...
                : isEs ? '14 especies pioneras reales de acera en un tira y afloja botánico por turnos. Presupuesto CSR equilibrado de 36 puntos, 6 crisis estacionales y custodia cívica.' : '14 real sidewalk pioneer species in a turn-based botanical tug-of-war. Balanced 36-point CSR budget, 6 seasonal crises, and civic stewardship.'}
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {isDe ? 'Duell-Arena' : isEs ? 'Arena de duelo' : 'Duel Arena'}
            </button>
            <button
            ...
            >
              ...
              {isDe ? '14-Arten-Kader' : isEs ? 'Plantilla de 14 especies' : '14-Species Roster'}
            </button>
            <button
            ...
            >
              ...
              {isDe ? 'Pacht & Samen-Markt' : isEs ? 'Custodia y semillas' : 'Stewardship & Seeds'}
            </button>
            <button
            ...
            >
              ...
              {isDe ? 'CSR-Regeln' : isEs ? 'Reglas CSR' : 'CSR Rules'}
            </button>
            ...
            {onOpenDose && (
              ...
              >
                <span>{isDe ? 'Dossier' : isEs ? 'Dosier' : 'Dossier'}</span>
            ...
            <div className="rounded-xl border-2 border-emerald-500/40 bg-emerald-50/30 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-emerald-800 flex items-center gap-1.5">
                  ...
                  {isDe ? 'Deine Pflanze (Pionier A)' : isEs ? 'Tu planta (pionera A)' : 'Your Plant (Pioneer A)'}
                </span>
            ...
            <div className="rounded-xl border-2 border-rose-500/40 bg-rose-50/30 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-rose-800 flex items-center gap-1.5">
                  ...
                  {isDe ? 'Kiez-Konkurrent (Pionier B)' : isEs ? 'Planta rival (pionera B)' : 'Rival Plant (Pioneer B)'}
                </span>
            ...
            <div className="flex items-center justify-between">
              <div>
                <span className="text-xs font-bold uppercase tracking-wider text-stone-500">
                  {isDe ? 'Deckungsbalken in der Asphaltritze (Tug-of-War)' : isEs ? 'Tira y afloja por la grieta' : 'Fissure Coverage Tug-of-War'}
                </span>
              ...
              <div className="rounded-xl border border-amber-300 bg-amber-50/60 p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-amber-900 uppercase tracking-wider font-mono">
                      {isDe ? currentEvent.monthDe : currentEvent.monthEn} · {isDe ? 'Saisonale Belastung' : isEs ? 'Crisis estacional' : 'Seasonal Crisis'}
                    </span>
                  </div>
                ...
                <div className="pt-2 border-t border-amber-200/60 space-y-2">
                  <label className="text-xs font-bold text-stone-800 block">
                    {isDe ? 'Wähle deine botanische Taktik für diesen Monat:' : isEs ? 'Elige tu respuesta táctica para este mes:' : 'Choose your tactical response for this month:'}
                  </label>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    ...
                    >
                      <div className="font-bold">⚖️ {isDe ? 'Ausgewogen' : isEs ? 'Equilibrada' : 'Balanced'}</div>
                      <div className="text-[10px] opacity-80">{isDe ? '+1 Grundwert' : isEs ? '+1 base' : '+1 Baseline'}</div>
                    </button>
                    <button
                    ...
                    >
                      <div className="font-bold">🪵 {isDe ? 'Wurzelbunker' : isEs ? 'Reserva en raíz' : 'Root Storage'}</div>
                      <div className="text-[10px] opacity-80">{isDe ? '+3 WURZEL/DÜRRE' : isEs ? '+3 RAÍZ/SEQUÍA' : '+3 ROOT/DROUGHT'}</div>
                    </button>
                    <button
                    ...
                    >
                      <div className="font-bold">🚀 {isDe ? 'Wachstumsspurt' : isEs ? 'Arranque rápido' : 'Speed Spurt'}</div>
                      <div className="text-[10px] opacity-80">{isDe ? '+3 TEMPO/SAAT' : isEs ? '+3 VELOCIDAD/SEMILLA' : '+3 SPEED/SEED'}</div>
                    </button>
                    <button
                    ...
                    >
                      <div className="font-bold">🧪 {isDe ? 'Giftbarriere' : isEs ? 'Defensa química' : 'Toxic Defense'}</div>
                      <div className="text-[10px] opacity-80">{isDe ? '+3 CHEMIE/TRITT' : isEs ? '+3 QUÍMICA/PISOTEO' : '+3 CHEM/TRAMPLE'}</div>
                    </button>
                  </div>
                </div>
              ...
              <div className={`rounded-xl p-5 border text-center space-y-3 ${
                ...
                <h3 className="text-xl font-bold font-serif-title">
                  {playerCoverage > 50
                    ...
                    : (isDe ? 'Ökologisches Gleichgewicht (50:50)' : isEs ? 'Equilibrio ecológico (50:50)' : 'Ecological Equilibrium (50:50)')}
                </h3>
                <p className="text-xs max-w-lg mx-auto leading-relaxed">
                  {playerCoverage > 50
                    ? (isDe
                      ...
                      : isEs ? 'Has asegurado la cobertura dominante en la junta del pavimento. ¡2 cartas de semilla añadidas a tu herbario de custodia!' : 'You secured dominant coverage in the pavement seam. 2 seed cards added to your stewardship herbarium!')
                    : (isDe
                      ? 'Die gegnerische Art konnte die extremen saisonalen Stressoren effektiver abfedern.'
                      : isEs ? 'La especie rival resistió los estreses estacionales con mayor resiliencia fenotípica.' : 'The rival species weathered seasonal stressors with higher phenotypic resilience.')}
                </p>
                ...
                >
                  ...
                  <span>{isDe ? 'Neues Duell starten' : isEs ? 'Revancha' : 'Start Rematch'}</span>
                </button>
              </div>
            )}
          </div>
          ...
          {battleLogs.length > 0 && (
            <div className="rounded-xl border border-stone-200 bg-white p-4 space-y-3 shadow-xs">
              <h4 className="text-xs font-bold uppercase tracking-wider text-stone-500 flex items-center gap-1.5">
                ...
                {isDe ? 'Rundenprotokoll & Mathematische Auflösung' : isEs ? 'Registro de rondas y resolución matemática' : 'Round Log & Mathematical Resolution'}
              </h4>
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-stone-200 bg-white p-6 space-y-4 shadow-xs">
            <h3 className="text-base font-bold font-serif-title text-stone-900 flex items-center gap-2">
              ...
              {isDe ? 'Pacht- und Chronisten-Modell (Stewardship statt Besitz)' : isEs ? 'Modelo de custodia' : 'Stewardship Custody Model'}
            </h3>
            ...
            <p className="text-xs text-stone-600 leading-relaxed">
              {isDe
                ...
                : isEs ? 'En Fugenduell, los organismos silvestres nunca pueden mercantilizarse, capturarse ni poseerse. La planta silvestre pertenece al procomún urbano. Quien juega actúa como cronista cívico que asume el cuidado de grietas concretas de la acera.' : 'In Fugenduell, wild organisms can never be commodified, captured, or owned. The wild plant belongs to the urban commons. Players act as civic chroniclers who adopt caretaking stewardship for specific sidewalk cracks.'}
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
              <div className="p-4 rounded-lg bg-amber-50/60 border border-amber-200">
                <span className="text-xs font-bold uppercase tracking-wider text-amber-900 block mb-1">
                  {isDe ? 'Verfügbare Samen-Karten' : isEs ? 'Cartas de semilla disponibles' : 'Available Seed Cards'}
                </span>
                <div className="text-2xl font-bold font-mono text-amber-950">
                  {collectedSeeds} <span className="text-xs font-normal text-amber-800">{isDe ? 'Samen im Beutel' : isEs ? 'Semillas en la bolsa' : 'Seeds in pouch'}</span>
                </div>
                <div className="text-[11px] text-amber-800 mt-2">
                  {isDe ? 'Gewinne Duelle oder logge reale Fotos in #Krautschau, um neue Samen freizuschalten.' : isEs ? 'Gana duelos o envía fotos verificadas con #Krautschau para ganar semillas.' : 'Win duels or submit verified photos via #Krautschau to earn seeds.'}
                </div>
              </div>

              <div className="p-4 rounded-lg bg-stone-50 border border-stone-200">
                <span className="text-xs font-bold uppercase tracking-wider text-stone-700 block mb-1">
                  {isDe ? 'Gepachtete Asphaltritzen (Stewardship)' : isEs ? 'Grietas bajo custodia' : 'Stewarded Fissures'}
                </span>
      ...
      {viewTab === 'rules' && (
        <div className="rounded-xl border border-stone-200 bg-white p-6 space-y-4 shadow-xs text-xs text-stone-700 leading-relaxed">
          <h3 className="text-base font-bold font-serif-title text-stone-900 flex items-center gap-2">
            ...
            {isDe ? 'Wissenschaftliche Grundlagen: Das Grime-Dreieck (CSR)' : isEs ? 'Base científica: marco CSR de Grime' : 'Scientific Basis: Grimes CSR Framework'}
          </h3>
          ...
          <div className="p-3 rounded-lg bg-stone-50 border border-stone-200 text-stone-600">
            ...
            {isDe
              ...
              : isEs ? 'Sin pagar para ganar, sin cajas de botín abusivas. Las estadísticas están limitadas por rasgos botánicos reales.' : 'Zero pay-to-win, zero predatory lootboxes. Stats are constrained by actual botanical traits.'}
          </div>
        </div>
      )}
    </div>
