    ...
    <div className="space-y-8 animate-fadeIn max-w-5xl mx-auto">
      ...
      <div className="rounded-2xl bg-rose-900/5 border border-rose-800/20 p-6 md:p-8 space-y-3">
        <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full bg-rose-100 text-rose-900 text-xs font-semibold">
          ...
          <span>{lang === 'de' ? 'Das Negativ-Archiv (_entsorgt.md)' : lang === 'es' ? 'Archivo Negativo (_entsorgt.md)' : 'Negative Archive (_entsorgt.md)'}</span>
        </div>
          ...
          >
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono-code font-bold uppercase text-rose-800 flex items-center gap-1.5">
                  ...
                  {lang === 'de' ? 'Entsorgt' : lang === 'es' ? 'Descartada' : 'Discarded'}
                </span>
              ...
              <div className="space-y-1">
                <span className="text-xs text-stone-600 block">
                  {lang === 'de' ? 'Die ursprüngliche Idee:' : lang === 'es' ? 'La idea original:' : 'The Original Idea:'}
                </span>
              ...
              <div className="p-4 rounded-xl bg-rose-50/70 border border-rose-200/80 text-xs sm:text-sm text-rose-950 space-y-1.5 leading-relaxed">
                <span className="font-bold font-mono-code uppercase tracking-wide block text-rose-900">
                  {lang === 'de' ? 'Warum entsorgt?' : lang === 'es' ? '¿Por qué descartada?' : 'Why Discarded?'}
                </span>
              ...
              {item.evidence && item.evidence.length > 0 && (
                <div className="pt-2">
                  <span className="text-xs text-stone-500 block font-mono-code uppercase">
                    {lang === 'de' ? 'Bestehende Nachweise:' : lang === 'es' ? 'Evidencia existente:' : 'Prior Art Evidence:'}
                  </span>
            ...
            <div className="mt-5 pt-3 border-t border-stone-100 space-y-1 text-xs text-stone-700 bg-stone-100/60 p-2.5 rounded-lg">
              <span className="font-bold text-stone-900 flex items-center gap-1">
                ...
                {lang === 'de' ? 'Die Lehre daraus:' : lang === 'es' ? 'La lección aprendida:' : 'The Key Lesson:'}
              </span>
              <p className="italic">
      ...
      <div className="p-6 rounded-2xl bg-stone-900 text-stone-200 text-xs sm:text-sm space-y-3 border border-stone-800">
        <h4 className="font-bold text-base font-serif-title text-white flex items-center gap-2">
          ...
          {lang === 'de' ? 'Die Philosophie des Streichens' : lang === 'es' ? 'La Filosofía del Descarte' : 'The Discipline of Discarding'}
        </h4>
        <p className="leading-relaxed">
          {lang === 'de'
            ? 'Regel 1 besagt: Ideen sind billig. Wer den Markt nicht prüft und Ideen verschenkt, die längst in GitHub-Trending standen, erzeugt nur Rauschen für den Empfänger. Das Negativ-Archiv ist der Beweis, dass der Filter arbeitet.'
            : lang === 'es'
            ? 'La Regla 1 dice: las ideas son baratas. Si no verificas el estado del arte y regalas ideas que ya fueron tendencia en GitHub, solo generas ruido para el destinatario. El archivo negativo demuestra la rigurosidad del filtro.'
