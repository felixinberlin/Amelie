...
Berlin · github.com/felixinberlin`
  ...
  },
  {
    ...
    date: '24. September 2026',
    reviewAfter: 'März 2027',
    recipientsDe: 'Katta Spiel, HCI-Gruppe der TU Wien (ERC ACCESSTECH, 2024–2029) — als Forschungssonde, mit Code · nachrangig: Eva Hornecker (Bauhaus-Universität Weimar, Mitautorin derselben CHI-Studie) · danach die Öffentlichkeit (Show HN), sobald das README hält, was der Code tut',
    recipientsEn: 'Katta Spiel, HCI group at TU Wien (ERC ACCESSTECH, 2024–2029) — as a research probe, with code · secondary: Eva Hornecker (Bauhaus-Universität Weimar, co-author of the same CHI study) · then the public (Show HN), once the README promises only what the code does',
    domain: 'tools',
    verdict: 'build_first',
    status: 'gepackt',
    ...
    whyNowEn: [
      ...
      'What is new is the place: a Workspace add-on in Apps Script runs entirely inside the user\'s Google account — events in their own calendar, history in a sheet in their own Drive, settings in per-user storage. For a study that means participants keep their data and share the sheet only if they want to.',
      'What is new is that it exists: v0.5.0, 38 tests green (under UTC, see first step), built in a day. What is given away is not the idea but the code.'
    ],
    ...
    firstStepDe: {
      ...
      criteria: 'Arbeitszeiten gelten in der Zeitzone der Nutzerin (heute läuft das Skript auf `Etc/UTC`: „9–20 Uhr" ist in Berlin 11–22 Uhr, und `npm test` ist nur mit `TZ=UTC` grün — unter Europe/Berlin fallen 5 von 38 Tests). Die Berechtigung `script.external_request` ist entfernt, weil nichts nach außen ruft. Das README behauptet nicht mehr „gleiche Eingaben, gleiches Ergebnis" (der Wurf nutzt `Math.random`, deterministisch ist er nur bei Chaos 0) und nicht mehr, die Prophezeiung sei bis zu ihrem Tag gesperrt (sie ist ein normaler Termin). Eine LICENSE-Datei liegt im Repo, und zwar CC0 — das README sagt MIT, und MIT verlangt den Copyright-Vermerk, also eine Bedingung, die der Pledge ausschließt.'
    },
    firstStepEn: {
      ticket: 'Fix the places where the code delivers less than the description promises — before anyone tests it on people.',
      criteria: 'Working hours apply in the user\'s time zone (the script currently runs on `Etc/UTC`: "9 am–8 pm" is 11 am–10 pm in Berlin, and `npm test` is only green with `TZ=UTC` — under Europe/Berlin 5 of 38 tests fail). The `script.external_request` scope is removed, since nothing calls out. The README no longer claims "same inputs, same output" (the roll uses `Math.random`; it is deterministic only at chaos 0) nor that the prophecy is locked until its day (it is an ordinary event). A LICENSE file is in the repo, and it is CC0 — the README says MIT, and MIT requires keeping the copyright notice, a condition the pledge rules out.'
    },
